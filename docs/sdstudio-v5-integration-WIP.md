...
3. **프롬프트·결과 흐름** — 시험용 씬의 연필 버튼 `중간 프롬프트 퀵 수정`을 열어 문구를 바꾸고 닫았다 다시 열어 값이 유지되는지 본다. 결과가 2장 이상 있는 같은 씬에서 썸네일 하나를 눌러 상세 이미지를 연 뒤, 큰 이미지 위를 가로로 50px 이상 왼쪽·오른쪽으로 쓸어 같은 씬의 이전·다음 결과로 한 장씩 바뀌는지 본다. 상세를 닫은 뒤 `이미지변형` 탭도 열려야 한다.
4. **Quick 생성·퀵 메뉴** — `퀵 생성` 탭에서 해상도를 선택하고 1장 생성한다. 결과가 Quick 히스토리에 나타나고 원래 프로젝트가 바뀌지 않아야 한다. 환경설정 `퀵 메뉴`에서 `퀵 메뉴 플로팅 버튼 표시`를 켜면 일반 화면 우하단에 노란 원형 번개 버튼이 보인다. 이 버튼을 짧게 탭하면 `퀵 메뉴`가 열리고, 버튼을 0.4초 이상 누른 채 끌어 다른 위치에서 손을 떼면 이동해야 한다. PWA를 다시 열어도 마지막 위치를 유지해야 한다.
...
6. **템플릿·일괄 생성** — 캐릭터 프리셋이 하나 이상 있는 시험용 프로젝트를 연다. 메인 화면에서 현재 프로젝트 이름이 보이는 도구 줄의 보라색 겹친 사각형 아이콘(기본 배치에서 이름 수정 연필과 백업/내보내기 아이콘 사이)을 눌러 `템플릿 관리`를 연다. 보이지 않으면 같은 줄의 `…` → `프로젝트 도구 더보기`에서 `씬 템플릿`을 누른다. `프로젝트 템플릿` → `새 템플릿`으로 이름을 만들고, 선택된 템플릿의 `캐릭터 프리셋` 구역에서 `현재 프로젝트에서 추가`로 캐릭터 하나를 넣는다. 이어 `씬 템플릿` → `현재 씬 전체로 만들기`로 씬 템플릿 하나를 만든다. 다시 프로젝트 템플릿을 선택하고 `일괄 생성`에서 대상 폴더를 입력한 뒤 `캐릭터 축` 1개와 `씬 축` 1개만 체크해 `1개 생성`을 누른다. `생성한 프로젝트 예약 등록`은 누르지 않는다. 생성된 프로젝트가 열리고 원본 시험 프로젝트의 씬·이미지가 그대로여야 한다.
...
### 6.1 첫 L3 결과와 프롬프트 후속 수정

- 사용자 실제 검사에서 나머지 시나리오는 정상으로 보고됐지만, 프롬프트 영역 3건 때문에 전체 L3는 통과 처리하지 않았다.
