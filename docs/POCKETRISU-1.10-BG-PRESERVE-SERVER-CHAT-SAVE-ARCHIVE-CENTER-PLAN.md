...
NAI Studio는 이번 계획서가 보관된 저장소이며, 실제 수정 대상은 **PocketRisu 개인 패처, 그 패처가 만드는 PocketRisu 실행 코드, AC JS·Go**랍니다.
...
**설계 판단:** 현재 검토 결과로는 패처만의 수정으로 전체 요구사항을 보장하는 안보다, **패처 + 최소 AC JS 호스트 연결 + AC Go의 요청별 설정·서버 관측 계약 보완**을 포함하는 안이 타당하답니다. 기존 source v1을 정직하게 재사용할 수 있는지는 최소 실험으로 판정하되, 설정 격리 문제는 그와 독립적으로 해결해야 한답니다.
