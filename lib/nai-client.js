...
class NaiClient {
  ...
  async login(email, password) {
    ...
    if (!res.ok) {
      // 서버가 준 실제 사유를 함께 노출 (NAI 측 오류/점검 등). JSON이면 message만 추출. (SDStudio 4.13 58c9640)
      let detail = '';
      try {
        const text = await res.text();
        try { detail = JSON.parse(text)?.message || text; } catch { detail = text; }
      } catch {}
      if (detail) console.error('NAI 로그인 실패:', res.status, detail);
  ...
  async getCredits() {
    ...
    const res = await fetch(this.imageEndpoint + '/user/data', {
      headers: {
  ...
  // 토큰이 NovelAI에서 실제로 유효한지 검증한다 (단순 파일 존재 확인이 아님). (SDStudio 4.13 630e0e5)
  // 200 → 'valid', 401/403(인증 거부) → 'invalid', 그 외·네트워크 오류 → 'error'(상태 유지).
  async validateToken(token) {
    const tok = token ?? this.token;
    if (!tok || !String(tok).trim()) return 'invalid';
    try {
      const res = await fetch(this.imageEndpoint + '/user/data', {
        headers: {
      ...
      return 'error'; // 5xx 등 일시 오류 → 상태 유지(오탐 방지)
    } catch (e) {
  ...
  async generateImage(params, config = {}) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (errText.includes('<!doctype') || errText.includes('<!DOCTYPE') || errText.includes('<html')) {
          errText = '(Cloudflare HTML error page)';
        } else if (errText.length > 200) {
  ...
  async augmentImage(params) {
    ...
    // audit B2 — generateImage와 동일하게 AbortController+timeout. 없으면 NAI 소켓 hang 시
    // /api/augment 핸들러가 영구 pending. 정상 응답은 그대로, hang만 abort(정상 완료보다 큰 캡).
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 180000);
    try {
  ...
  async encodeVibeImage(params, config = {}) {
    ...
    // audit B2 — generateImage와 동일하게 AbortController+timeout. 없으면 NAI 소켓 hang 시
    // serverEncodeVibe→fillBatchInBackground에서 그 배치 fill이 영원히 안 끝남. encode는
    // generate보다 가벼워 120s 캡(정상 완료보다 큼). 정상 응답 동작 보존.
    const controller = new AbortController();
