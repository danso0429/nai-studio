...
class NaiClient {
  ...
  async generateImage(params, config = {}) {
    ...
    console.log('[NAI payload]', JSON.stringify({
      model: body.model,
      action: body.action,
      vibesCount: body.parameters?.reference_image_multiple?.length || 0,
      charRefsCount: body.parameters?.director_reference_images?.length || 0,
      charPromptsCount: body.parameters?.characterPrompts?.length || 0,
    }));
  ...
  async augmentImage(params) {
    ...
    const entries = Object.keys(zip.files);
    if (!entries.length) throw new Error('Empty zip response');
    return await zip.file(entries[entries.length - 1]).async('base64');
  }
