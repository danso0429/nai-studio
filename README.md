...
- **🕘 최근 생성 히스토리** — 최근 4시간 결과를 2열로 확인하고 프로젝트·씬으로 바로 이동. PC에서는 우측 패널, 모바일에서는 이동 가능한 핸들과 드로어로 열며 환경설정에서 썸네일 크기 조절
- **⚡ 퀵 생성** — 전용 탭에서 1장 생성과 자동 연속 생성을 버튼으로 구분. 실행 중인 모드의 같은 버튼으로 취소하고, 반대 버튼으로 안전하게 모드 전환
...
| **최근 생성 히스토리** | 최근 결과 패널 | 최근 4시간·2열 패널/모바일 드로어, 크기 조절·프로젝트/씬 이동·즐겨찾기 |
| **퀵 생성** | 퀵 생성 모드 | 모바일에도 맞춘 전용 탭, 1장/자동 생성 분리 버튼과 모드별 취소·전환 |
