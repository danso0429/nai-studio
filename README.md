...
- **캐릭터 프리셋** — 카드 목록 + 대표 이미지 + 상세 설정. 여러 캐릭터를 한 번에 적용하고 폴더로 정리하며, 프리셋 × 씬 자동 순회 생성과 이미지 포함 내보내기/불러오기 지원
...
- **프로젝트·폴더·씬 템플릿** — 프롬프트·스타일·캐릭터·참조 이미지·씬 구성을 템플릿으로 저장하고 폴더 상속. 캐릭터 × 씬 조합을 미리 계산해 여러 프로젝트를 일괄 생성
- **프롬프트 확장** — 상위·추가·중간 프롬프트를 접어 관리하고 씬 카드에서 중간 프롬프트를 빠르게 수정. 모든 관련 입력에서 chunk 사용
- **결과 탐색** — 상세 이미지 좌우 스와이프, 이전·다음 씬 이동, 빠른 연속 전환 중 오래된 응답 차단
- **테마 설정** — 다크/트루 다크/화이트를 포함한 6개 템플릿과 배경·표면·강조색 직접 조절
...
- **🕘 최근 생성 히스토리** — 최근 30장 결과를 서버에 유지하고 프로젝트·씬으로 바로 이동. PC에서는 우측 패널, 모바일에서는 이동 가능한 핸들과 드로어로 열며 환경설정에서 썸네일 크기 조절
- **⚡ 퀵 생성** — 숨겨진 전용 프로젝트에 결과를 분리하고, 전용 탭에서 1장 생성과 자동 연속 생성을 구분. 실행 중인 모드의 같은 버튼으로 취소하고 반대 버튼으로 안전하게 전환
- **🧩 화면 배치·퀵 메뉴** — classic/compact/sidebar/modern 배치, 툴바 순서·숨김·영역 이동, `Ctrl+K` 또는 이동 가능한 플로팅 버튼으로 공용 기능 빠른 실행
- **🔒 여러 화면 편집 보호** — 브라우저 탭·기기 사이 프로젝트 소유권을 서버가 관리. 다른 화면은 읽기 전용 미러로 열고 생성 예약·즐겨찾기 같은 허용 작업만 위임
- **🖼️ WebP 변환** — 서버에서 씬·프로젝트 이미지를 WebP로 변환하고 NAI 메타데이터와 참조 저장을 확인한 뒤에만 원본 PNG 삭제. 자동 변환은 선택사항
- **🗄️ 저장소 v2** — 프로젝트 JSON과 이미지 6종을 한 workspace로 묶는 서버 권위 저장 구조. 백업·재개·rollback을 제공하며 사용자가 명시적으로 선택하기 전에는 데이터를 이동하지 않음. 전환 뒤 PWA를 다시 열어도 마지막 프로젝트 소유권을 자동 복구
...
- **🛡️ 데이터 보호** — 같은 파일의 동시 저장을 직렬화하고 삭제·이름 변경·가져오기·복원 전 저장을 확정. 폴더만 삭제할 때 프로젝트 이동이 하나라도 실패하면 영구 삭제를 중단하며, 읽기 실패를 빈 데이터로 오인한 덮어쓰기도 차단
...
| **통합 기준** | SDStudio 5.0.0 | SDStudio 5.0.0 기능 효과 + Remote 서버/PWA 적응 |
...
| **최근 생성 히스토리** | 최근 30장 로컬 기록 | 최근 30장 서버 영속·2열 패널/모바일 드로어, 프로젝트·씬 이동·즐겨찾기 |
| **퀵 생성** | 퀵 생성 모드 | 숨김 전용 프로젝트 + 모바일 전용 탭, 1장/자동 생성 분리와 모드별 취소·전환 |
| **프로젝트 템플릿** | 프로젝트·폴더·씬 템플릿과 일괄 생성 | 동일 효과 + 독립 이미지 소유권·서버 큐 별도 예약 |
| **화면 배치** | 툴바·패널 배치 편집, Quick menu | 동일 효과 + Remote 도구·모바일 classic 안전 폴백 |
| **동시 화면** | Electron 다중 창 lock/미러 | 브라우저 탭·기기별 서버 lease와 읽기 전용 미러 |
...
| **홈 화면 앱(PWA) 복귀** | (데스크톱 앱) | iOS에서 백그라운드 후 새로고침돼도 마지막 프로젝트·탭과 v2 편집 소유권 자동 복원 |
| **저장·삭제·가져오기 보호** | 로컬 앱 파일 작업 | 서버 저장 직렬화 + 안정 snapshot + 폴더 보존 삭제 검증 + 불확실한 읽기 오류 시 원본 저장 차단 |
| **이미지 최적화** | WebP 변환 | 서버 WebP/AVIF 변환 + NAI stealth/Comment 보존 확인 뒤 원본 정리 |
| **저장소 v2** | 로컬 workspace와 migration | 서버 ledger 기반 명시적 migration·재개·backup·rollback + PWA 콜드 시작 lease 복구 |
| **커스텀 해상도** | 모달 1개 | 한 폼에 가로/세로 동시 입력 + 자동 보정 |
| **테마** | 기본 테마 설정 | 6개 템플릿 + 배경·표면·강조색 직접 조절 |
...
- **v1.13** — SDStudio 5.0.0의 134개 upstream 변경을 전수 해소. 템플릿·일괄 생성, 화면 배치·퀵 메뉴, 추가/퀵 프롬프트, 결과 스와이프, 최근 30장 히스토리, WebP, 다중 화면 lease·읽기 전용 미러, 명시 선택 저장소 v2를 Remote 구조에 맞게 통합. runtime-static 순환 의존도 SCC·내부 edge·직접 pair 모두 0으로 정리. v1.13.1에서 저장소 전환 뒤 PWA 콜드 시작의 반복 reload·project-owner 409를 수정.
- **v1.12** — SDStudio 4.14.1 선택 통합. 최근 4시간 이미지 히스토리, 1장/자동 버튼을 분리한 퀵 생성, 6개 템플릿과 사용자 색상 테마를 추가. 저장 직렬화와 안정 snapshot으로 삭제·이름 변경·가져오기·복원 중 데이터 부활·덮어쓰기 경로를 차단하고, 모바일 드로어·활성 탭·이동 가능한 히스토리 핸들을 정리.
