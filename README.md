...
- **🗄️ 저장소 v2** — 프로젝트 JSON과 이미지 6종을 한 workspace로 묶는 서버 권위 저장 구조. 백업·재개·rollback을 제공하며 사용자가 명시적으로 선택하기 전에는 데이터를 이동하지 않음
...
| **저장소 v2** | 로컬 workspace와 migration | 서버 ledger 기반 명시적 migration·재개·backup·rollback |
...
- **v1.13** — SDStudio 5.0.0의 134개 upstream 변경을 전수 해소. 템플릿·일괄 생성, 화면 배치·퀵 메뉴, 추가/퀵 프롬프트, 결과 스와이프, 최근 30장 히스토리, WebP, 다중 화면 lease·읽기 전용 미러, 명시 선택 저장소 v2를 Remote 구조에 맞게 통합. runtime-static 순환 의존도 SCC·내부 edge·직접 pair 모두 0으로 정리.
