...
| **프롬프트 chunk** | 없음 | 이름 붙인 태그 묶음 알약 — 칸에 커서 위치 삽입/생성 시 펼침, 원자화·경계 자동 쉼표·데스크탑 호버 미리보기 + 샘플링 프리셋 별도 저장. 복사 시 chunk 포함/제외 (v1.9.0). 기존 그림체(프롬프트) 프리셋은 chunk가 대체·폐기. **캐릭터 프롬프트/네거티브 칸에도 +chunk** (v1.9.2) |
...
| **씬 복사** | v4.9.0 추가 (다른 프로젝트) | 동일 (v1.8.2) — 개별 + 일괄, 설정만 또는 이미지 포함 |
