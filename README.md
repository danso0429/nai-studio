...
- **🆕 캐릭터 프리셋 v2** (v1.6.0, SDStudio v4.8.0 흡수 + v1.7.3에서 v4.8.1/v4.8.2 통합) — 카드 그리드 목록 + 중앙 모달 편집 + 대표 이미지(폴백 체인) + 상세 슬라이더(IS/RS, Strength/Fidelity, 타입/활성화 토글). **JSON Import/Export** (바이브/레퍼런스/대표이미지 base64 포함) + **순차 생성** (프리셋 N × 씬 M 자동 순회, 일시정지/재개/취소, 프리셋 전환 쿨다운). **v1.8.2** (v4.9.0 흡수): 다중 캐릭터 지원 — 프리셋 적용 시 기존 바이브/레퍼런스 유지 + fromPreset 태그로 프리셋 것만 추가/제거. 캐릭터 위치 지정 좌표평면 드래그 UI. 바이브 이미지 손상 시 건너뛰기 + 안내 메시지.
- **🆕 프로젝트 브라우저** (v1.8.2) — 카드 그리드 뷰로 프로젝트 탐색. 썸네일, 검색, 최근 프로젝트, 즐겨찾기 정렬. 다른 프로젝트로 씬 복사 (개별 + 일괄, 설정만 또는 이미지 포함).
- **🎨 트루 다크 모드** (v1.8.2) — 순수 검정 배경. 다크/트루 다크/화이트 3택. 메인 앱 + queue.html 모두 적용.
- **🆕 프롬프트 chunk 시스템** (v1.9.0) — 이름 붙인 순수 태그 묶음(전역 저장, NovelAI prompt chunk식). `설정 > 프롬프트 chunk`에서 관리(폴더·2D 색상 피커), 프롬프트 칸에 **알약**으로 커서 위치 삽입 → 생성 시 content로 펼침. 알약 원자화(커서 이동·Backspace 2단 삭제), 알약 경계 자동 쉼표, chunk 삭제 시 죽은 토큰 자동 정리, 데스크탑 호버 시 내용 미리보기. **샘플링 프리셋**(스텝/가이던스/리스케일/샘플러/노이즈스케줄) 별도 저장·적용/해제. 다른 프로젝트로 프롬프트 복사 시 폴더 트리 + chunk 포함/제외 선택. *기존 그림체(프롬프트) 프리셋은 chunk + 샘플링 프리셋이 대체하며 v1.9.0에서 폐기.*
- **🆕 조합 에디터 drag UX + 씬 토글 그룹** (v1.9.0) — 조합 슬롯을 **⠿ 핸들**로 잡아 다른 열(빈 열 포함)로 자유 드래그, 끝 빈 공간에 떨어뜨리면 **새 열 자동 생성**, 빈 열 삭제 버튼. **씬 토글 그룹**: 조합 piece의 충돌 태그(예: nude 복장과 부딪히는 `fully clothed female`)를 묶어 버튼으로 on/off — OFF면 생성 시 그 태그만 빠짐(piece 원본 불변). 그룹 정의는 **씬 이름 기준 전역 공유**(같은 이름 씬이면 다른 프로젝트에도 보임), on/off는 프로젝트별.
- **🆕 프롬프트 찾기** (v1.9.1) — 검색창(데스크탑 `Ctrl+Shift+F` / 모바일 돋보기)에서 씬 이름 필터 + **상위/하위/네거티브 프롬프트 칸의 검색어 태그를 형광 강조**(chunk 알약 속 태그면 알약 외곽선). 매칭 개수 X/Y + 다음/이전(`Enter` / `Shift+Enter`)으로 그 위치 스크롤. 씬 목록은 이름만 검색.
- **🆕 캐릭터 프롬프트 chunk + 네거티브 칸 자동완성** (v1.9.2) — **캐릭터 프롬프트 / 캐릭터 네거티브 프롬프트** 칸에도 **+chunk 버튼** 추가(메인 프롬프트처럼 chunk 알약 커서 삽입 → 생성 시 펼침; 씬 에디터 캐릭터 + 프리셋/조합 에디터 캐릭터 둘 다). **조합 네거티브 / 씬 전용 네거티브** 칸을 단순 입력에서 **db.csv 태그 자동완성 에디터**로 교체.
- **🆕 씬 결과 화면 UX + chunk 입력 편의** (v1.9.3) — 데스크탑에서 **Ctrl 누르는 동안** 이미지 선택모드(떼면 해제), 씬 결과 상단 **제목 클릭 시 씬 이름 inline 편집**, 우클릭 "다른 씬으로 이미지 **이동**"(복사 아님 — 원본 삭제, 생성정보 + 즐겨찾기 유지), "다른 프로젝트로 씬 복사" 대상 선택을 **폴더 그룹 dropdown**으로, 조합 에디터 첫 행 삭제 제한 완화(조각 1개일 때만 차단). chunk 관리 **태그(content) 칸에 db.csv 자동완성** + chunk 알약 뒤/앞 **paste 시 자동 쉼표**.
...
- **🖱️ 데스크탑 ctrl 선택** (v1.8.0) — ctrl(Win/Linux)·cmd(macOS)+좌클릭으로 이미지 선택 모드 자동 진입. **v1.9.3**: ctrl을 누르고 있는 동안 선택모드 유지, 떼면 해제

---

## SDStudio PC 버전과의 차이점

> 기준: SDStudio v4.9.0 (v1.9.0 시점). 현재 Remote 버전은 `version.json` 참조 (변경 로그는 `CHANGELOG.md`).

| 항목 | SDStudio PC (v4.9.0) | SDStudio Remote |
| --- | --- | --- |
...
| **프롬프트 chunk** | 없음 | 이름 붙인 태그 묶음 알약 — 칸에 커서 위치 삽입/생성 시 펼침, 원자화·경계 자동 쉼표·데스크탑 호버 미리보기 + 샘플링 프리셋 별도 저장. 복사 시 chunk 포함/제외 (v1.9.0). 기존 그림체(프롬프트) 프리셋은 chunk가 대체·폐기. **캐릭터 프롬프트/네거티브 칸에도 +chunk** (v1.9.2). **chunk 관리 content 칸 db.csv 자동완성 · 알약 뒤/앞 paste 시 자동 쉼표** (v1.9.3) |
| **네거티브 칸 자동완성** | 메인 네거티브만 | 조합 네거티브 + 씬 전용 네거티브 칸도 db.csv 태그 자동완성 (v1.9.2) |
| **조합 에디터 / 토글 그룹** | 기본 슬롯 조합 | ⠿ 핸들로 슬롯 자유 드래그(빈 열 포함)·끝 빈 공간 드래그 시 새 열·빈 열 삭제 + **씬 토글 그룹**(충돌 태그 묶어 on/off, OFF면 생성 시 제거). 그룹 정의는 씬 이름 기준 전역 공유, on/off는 프로젝트별 (v1.9.0) |
| **프롬프트 찾기** | 없음 | 검색창에서 씬 이름 + 상위/하위/네거티브 칸 태그 형광 강조(chunk 알약 포함) · 매칭 X/Y · 다음/이전 스크롤 (v1.9.1) |
| **캐릭터 프리셋 다중 캐릭터** | v4.9.0 추가 (fromPreset 태그) | 동일 (v1.8.2) — 기존 바이브/레퍼런스 유지 + 프리셋 것만 추가/제거 |
| **프로젝트 브라우저** | v4.9.0 추가 (카드 그리드) | 동일 (v1.8.2) — 썸네일, 검색, 최근, 즐겨찾기 |
| **테마** | 다크 / 화이트 2택 | 다크 / 트루 다크(순수 검정) / 화이트 3택 (v1.8.2) — queue.html도 자동 적용 |
| **씬 복사** | v4.9.0 추가 (다른 프로젝트) | 동일 (v1.8.2) — 개별 + 일괄, 설정만 또는 이미지 포함. **대상 프로젝트 선택 폴더 그룹 dropdown** (v1.9.3) |
| **씬 결과 화면 조작** | 기본 | 데스크탑 Ctrl 홀드 선택모드(떼면 해제), 상단 제목 클릭 씬 이름 inline 편집, 우클릭 "다른 씬으로 이미지 **이동**"(원본 삭제, 생성정보+즐겨찾기 유지) (v1.9.3) |
