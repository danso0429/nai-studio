...
# 큐 활성 체크 (선택)
if command -v pm2 &> /dev/null && pm2 list 2>/dev/null | grep -q nai-studio; then
    QUEUE=$(curl -s localhost:6247/api/queue/status 2>/dev/null || echo '{}')
